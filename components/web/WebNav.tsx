...
function MenuBars() {
  return (
    <View className="gap-[4px] items-center">
      <View className="w-[16px] h-[2px] rounded-full bg-navy" />
      <View className="w-[16px] h-[2px] rounded-full bg-navy" />
      <View className="w-[16px] h-[2px] rounded-full bg-navy" />
    </View>
  );
    ...
    <View className="w-full items-center px-4 md:px-8 pt-[14px]">
      <View className="w-full max-w-[1200px]">
        <View className="flex-row items-center gap-6 rounded-full bg-navy-elevated border-[3px] border-white/[0.14] pl-[16px] pr-[7px] py-[7px] shadow-[0_12px_30px_rgba(4,12,30,0.42)]">
          ...
          <Pressable
            onPress={() => setOpen((v) => !v)}
            className={`lg:hidden w-[42px] h-[42px] rounded-full bg-white items-center justify-center ${FOCUS}`}
          ...
          >
            ...
            {open ? <CloseIcon size={17} color="#0B132B" /> : <MenuBars />}
          </Pressable>
        </View>

...
- **Blocked on you:** an App Password for a dedicated `noreply@yieldd.co` Google Workspace user. See PENDING #12 for the exact steps. Until that exists the function cannot be tested, and an untested function that emails real customers is not something to switch on.
- **Sending is Google Workspace SMTP**, decided 2026-08-31 — no new vendor, no DNS changes, and 2,000 messages/day against Resend's free-tier 100/day. Note that *Google Cloud* itself has no email service; this is Workspace. Send as a **dedicated user**, never `care@yieldd.co`: hitting the daily cap suspends that account's sending for 24 hours, and the support inbox must not be collateral.
- **Batch the sends.** One message per customer per week means the whole run fires at once; that both approaches the daily cap and looks like spam to a receiving server. Spread them.

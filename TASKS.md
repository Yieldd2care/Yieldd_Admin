...
- **Blocked on you:** a Google App Password for `care@yieldd.co`. See PENDING #12 for the exact steps. Until it exists the function cannot be tested, and an untested function that emails real customers is not something to switch on.
- **Sending is Google Workspace SMTP as `care@yieldd.co`**, decided 2026-08-31 — no new vendor, no DNS changes, no extra seat, and 2,000 messages/day against Resend's free-tier 100/day. (*Google Cloud* itself has no email service; this is Workspace.)
- **Three safeguards are part of the task, not optional**, because the sending account is also the support inbox and a cap breach suspends it for 24 hours. The cap is far away at real volume — the danger is a bug, so it is handled in code:
  1. a **hard per-run cap** the loop cannot exceed;
  2. **once per org per week enforced in the database** (`last_digest_sent_at`), so a retry or a re-run cannot double-send — the same replay-collides rule the device-generated lead ids follow;
  3. **throttled sends**, not one burst: 200 identical messages in a second reads as spam regardless of the cap.
